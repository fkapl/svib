...
def main(args):
    ...
    for i in range(start, stop):
        ...
        basename_prefix = f'{idx:0>6d}_'
        output_path = output_base_path / chunk
        output_path.mkdir(exist_ok=True)
        output_prefix_path = output_path / basename_prefix

        num_objects = random.randint(args.min_objects, args.max_objects)
        object_binds = [random.choice(binds) for _ in range(num_objects)]
        while True:
            try:
                TRIES += 1
                render_scene(args, num_objects, output_prefix_path, idx, object_binds)
                break
...
def render_scene(args,
    ...
    #print(f'object binds {object_binds}')
    ...
    output_image = Path(str(output_prefix_path) + 'image.png')
    ...
    objects, blender_objects = add_binds_objects(args, metadata['directions'], num_objects, scene.camera,
                                                  sampling_conf, object_binds)
    ...
    for i, (obj, meta) in enumerate(zip(blender_objects, objects), start=1):
        ...
        #print(f'Object  {i} with mask {obj_mask_path}')
        obj_mask_imgs.append(obj_mask_path)

        # What does color do?
        meta['color'], rgba = sampling_conf.sample_color_for_shape(meta['shape'])
        # Get material from object_binds
        meta['material'], mat_path = sampling_conf.get_material(object_binds[i - 1][2])
    ...
    mask_out_path = Path(str(output_prefix_path) + 'mask.png')
...
#
    ...
    if args.npz:
        npz_output_path = Path(str(output_prefix_path) + 'data.npz')
    ...
    with Path(str(output_prefix_path) + 'properties.json').open('w') as f:
...
def get_binds_objects(args, directions, num_objects, conf, object_binds):
    positions = []
    objects = []
    paths = []
    for i in range(num_objects):
        # Get size from bind
        size_name, r = conf.get_size(object_binds[i][1])
        obj_name_out, obj_path = conf.get_shape(object_binds[i][0])
...
def add_binds_objects(args, directions, num_objects, camera, conf, object_binds):
    """
    Add random objects to the current blender scene
    """
    objects, positions, paths = get_binds_objects(args, directions, num_objects, conf, object_binds)
    blender_objects = []
...
if __name__ == '__main__':
    ...
    # TODO: check how many tiles are good e.g. 256 or 512
